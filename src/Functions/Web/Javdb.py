...
def scrape_from_db(jav_file, jav_model, url_db, proxy_db):
    ...
    # 用户没有指定网址，则去搜索
        ...
        # 车头的第一页
        url_pref = f'{url_db}/video_codes/{pref_current}?locale=zh'
        ...
        if not list_cars:
            ...
            # 预估当前车牌 所在页面
            if no_page > 1:
                url_page = f'{url_db}/video_codes/{pref_current}?locale=zh&page={no_page}'
            ...
            if suf_max_n >= suf_current >= suf_min_n:
                ...
                while True:
                    ...
                    if no_page == 0:
                        return ScrapeStatusEnum.db_not_found, []
                    url_page_next = f'{url_db}/video_codes/{pref_current}?locale=zh&page={no_page}'
    ...
    # 得到 javdb
    url_jav_db = f'{url_db}/v/{javdb}?locale=zh'
